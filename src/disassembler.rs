...
impl Instruction {
    fn definitions(&self) -> impl Iterator<Item = Register> {
        (match self {
            ...
            Instruction::Mtlo(_) => None,
            Instruction::Dsllv(a, _, _) => Some(*a),
            Instruction::Dsrav(a, _, _) => Some(*a),
            Instruction::Dsrlv(a, _, _) => Some(*a),
            Instruction::Mult(_, _) => None,
            Instruction::Multu(_, _) => None,
            Instruction::Div(_, _) => None,
            Instruction::Divu(_, _) => None,
            Instruction::Add(a, _, _) => Some(*a),
            Instruction::Addu(a, _, _) => Some(*a),
            Instruction::Sub(a, _, _) => Some(*a),
            Instruction::Subu(a, _, _) => Some(*a),
            Instruction::And(a, _, _) => Some(*a),
            Instruction::Or(a, _, _) => Some(*a),
            Instruction::Xor(a, _, _) => Some(*a),
            Instruction::Nor(a, _, _) => Some(*a),
            Instruction::Mfsa(a) => Some(*a),
            Instruction::Mtsa(_) => None,
            Instruction::Slt(a, _, _) => Some(*a),
            Instruction::Sltu(a, _, _) => Some(*a),
            Instruction::Dadd(a, _, _) => Some(*a),
            ...
            Instruction::Dsra32(a, _, _) => Some(*a),
...
pub fn disassemble(data: u32) -> Instruction {
    ...
    match opcode {
        0b000000 => match data & 0b111111 {
            ...
            0b111111 => Instruction::Dsra32(rd, rs, shamt),
